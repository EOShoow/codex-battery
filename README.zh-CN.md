...
预测只使用当前周 reset 对应的样本，先把重复快照合并为 5 分钟桶，再融合“本周截至目前的稳定平均”和“约最近 24 小时节奏”；样本稀疏时会扩大近期观察区间。界面会按可用样本跨度和真实变化次数显示低、中、高置信度。这样可以去掉原先固定 `8h/天` 的假设，也能降低短时突发对结果的干扰。
...
这是非官方的本地看板。5 小时和 1 周额度优先读取原生 Codex UI 使用的本机 app-server 账号额度路径；今日消耗和 Top 来自本机 rollout 日志。周预测会把 rollout 历史与最新实时周用量合并，因此当 Codex 尚未落盘近期事件时，历史走势仍可能滞后。预测置信度用于说明可用历史量，它反映当前节奏的估计，不代表额度保证。
