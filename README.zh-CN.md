...
- 菜单详情：重置时间、可用完整重置次数及到期日期、今日 token 消耗、周预算预测、当前消耗最高的 Codex 对话、近期后台活动、数据生成时间
...
预测会把当前周 reset 对应的额度快照合并为 5 分钟桶，同时从此前 28 个完整自然日的本机活动时间提取 15 分钟活跃桶，形成“星期 × 小时”的个人使用画像。当前消耗按这张画像换算为活跃节奏，再融合约最近 24 小时的变化；夜间和历史空闲时段使用更低权重，不会继续照搬白天速率。右侧会显示置信度和历史天数，例如 `中 · 历史28d`；历史不足时自动回退到仅按当前窗口估算。
...
这是非官方的本地看板。5 小时和 1 周额度优先读取原生 Codex UI 使用的本机 app-server 账号额度路径；今日消耗和 Top 来自本机 rollout 日志。周预测只在本机内存中使用分桶后的活动时间和额度变化，不保存对话正文或标题画像；当 Codex 尚未落盘近期事件时，历史走势仍可能滞后。预测置信度用于说明可用历史量，它反映当前节奏的估计，不代表额度保证。
