...
- 外圈：1 周额度剩余
- 内圈：5 小时额度剩余
- 菜单详情：重置时间、今日 token 消耗、周预算预测、当前消耗最高的 Codex 对话、近期后台活动、最近一次本地检查时间
...
`更新于` 是本地检查时间，不是额度重置时间。如果 5 小时或 1 周窗口的重置时间已经过去，但 Codex 还没有写入新的 usage 事件，Codex Battery 会把这个窗口视为已重置，显示 `100%` 和 `已重置`。
...
- 点击 `刷新` 时
- 空闲时每 5 分钟刷新
- 检测到近期 Codex 活动时每 60 秒刷新

为了降低功耗，它只检查最近活跃的线程，并且只读取每个 rollout 日志的尾部，不会每次从头扫描所有日志。
